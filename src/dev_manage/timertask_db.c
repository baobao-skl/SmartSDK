...
static void CreateDatabaseTable(void)
{
	char *sql = "create table timertasktable(F_ID INTEGER PRIMARY KEY AUTOINCREMENT,F_MAC VARCHAR(30),F_NAME VARCHAR(50),F_PERIOD INTEGER,F_HAPPENTIME VARCHAR(20),F_ACTION INTEGER,F_TIPINFO INTEGER,F_HOWLONG INTEGER);" ;
...
BOOL AddOneTimerTask(timertask_item_t item)
{
	...
	sprintf(sql,"insert into timertasktable(F_MAC,F_NAME,F_PERIOD,F_HAPPENTIME,F_ACTION,F_TIPINFO,F_HOWLONG) values('%s','%s',%d,'%s',%d,%d,%d)",item.dev_mac,item.dev_name,item.period,item.happen_time,item.action,item.tipinfo,item.howlong);
...
void GetValidThingsToDo(const char *curr_time,const unsigned char weekday,unsigned char *nums,timertask_item_t *items)
{
	...
	const char *shorttime = curr_time + 11;
...
void GetAllTimerTaskInfo(void)
{
	...
	char *sql = "select * from timertasktable";
	int row_num = 0,column_num = 0,i = 0;
	char **azResult;
	sqlite3_get_table(db_timertask,sql,  &azResult, &row_num, &column_num, NULL);
	...
	if(row_num >= 1){
		...
		for(i = 1;i<row_num+1;i++)
		{
			sprintf(buffer,"{\"id\":\"%s\",\"mac\":\"%s\",\"name\":\"%s\",\"period\":\"%s\",\"happentime\":\"%s\",\"action\":\"%s\",\"tipinfo\":\"%s\",\"howlong\":\"%s\"},",
